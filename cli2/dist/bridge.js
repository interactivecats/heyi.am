...
function entriesToTurns(entries, cwd) {
    const turns = [];
        ...
        if (entry.type === "user") {
            ...
            for (const block of blocks) {
                if (block.type === "text") {
                    ...
                    if (toolInput)
                        toolInput = stripHomePath(toolInput, cwd);
                    turns.push({
                        timestamp: entry.timestamp,
                        type: "tool",
...
function computePerFileChanges(toolCalls, locStats, cwd) {
    const files = new Map();
...
function extractRawLog(entries, cwd) {
    const log = [];
    for (const entry of entries) {
        if (entry.type === "user") {
            const content = entry.message?.content;
 ...
 */
export function childDedupeKey(agentRole, startTime) {
...
export function toAgentChild(session) {
    return {

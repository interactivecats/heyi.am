...
function underThreshold(value: number, threshold: number): number {
  if (value >= threshold) return 0;
...
export const PRIMARY_ARCHETYPES: Archetype[] = [
  {
    id: "night-owl",
    name: "The Night Owl",
  ...
  },
  {
    ...
    score: (s) =>
      (overThreshold(s.corrections, 10) + overThreshold(s.override_success_rate, 0.6)) / 2,
  },
  {
    id: "delegator",
    name: "The Delegator",
  ...
  },
  {
    ...
    score: (s) =>
      (underThreshold(s.read_write_ratio, 1.5) + overThreshold(s.bash_commands, 50)) / 2,
  },
  {
    id: "overthinker",
    ...
    score: (s) =>
      (overThreshold(s.avg_prompt_words, 80) + overThreshold(s.question_rate, 0.4)) / 2,
  },
  {
    id: "speed-runner",
    ...
    score: (s) =>
      (overThreshold(s.turn_density, 3) + underThreshold(s.avg_prompt_words, 20)) / 2,
  },
  {
    id: "debugger",
    name: "The Debugger",
  ...
  },
  {
    ...
    score: (s) =>
      (overThreshold(s.please_rate, 0.4) + underThreshold(s.corrections, 3)) / 2,
  },
  {
    id: "architect",
    name: "The Architect",
    ...
    score: (s) =>
      (overThreshold(s.read_write_ratio, 5) + overThreshold(s.avg_prompt_words, 50)) / 2,
  },
  {
    id: "pair-programmer",
    ...
    score: (s) =>
      (overThreshold(s.turn_density, 1.5) + overThreshold(s.corrections, 10)) / 2,
  },
  {
    id: "marathon-runner",
    ...
    score: (s) =>
      (overThreshold(s.total_duration_min / Math.max(1, s.session_count), 120) +
  ...
  },
  {
    ...
    score: (s) =>
      (overThreshold(s.test_runs, 50) + overThreshold(s.question_rate, 0.3) +
  ...
  },
  {
    ...
    score: (s) =>
      (overThreshold(s.redirects_per_hour, 5) + overThreshold(s.corrections, 15) +
  ...
  },
  {
    ...
    score: (s) =>
      (overThreshold(s.agent_spawns, 10) + overThreshold(s.plan_mode_uses, 5)) / 2,
  },
  {
    id: "minimalist",
    name: "The Minimalist",
    ...
    score: (s) =>
      (underThreshold(s.avg_prompt_words, 12) + overThreshold(s.one_word_turn_rate, 0.2)) / 2,
  },
  {
    id: "secret-spiller",
...
export const MODIFIER_TRAITS: Modifier[] = [
  {
    id: "says-please",
...
export function matchArchetype(stats: VibeStats): ArchetypeMatch {
  // 1. Score all primaries where ALL conditions pass
  let bestPrimary: Archetype | null = null;
  let bestScore = -1;
  ...
  for (const modifier of MODIFIER_TRAITS) {
    if (impliedStats.has(modifier.statKey)) continue;
    if (!modifier.condition(stats)) continue;

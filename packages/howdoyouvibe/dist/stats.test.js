...
const { EXPLETIVE_RE, CORRECTION_START_RE, CORRECTION_PHRASE_RE, POLITE_RE, REASONING_RE, TEST_CMD_RE, SCOPE_CREEP_RE, APOLOGY_RE, SECRET_LEAK_RE, SECRET_EXAMPLE_RE, AI_ADMISSION_RE, COMPACTION_RE, } = _patterns;
// ─── Regex pattern tests ─────────────────────────────────────────────────
...
describe("COMPACTION_RE", () => {
    it("matches compaction summary", () => {
        expect(COMPACTION_RE.test("This session is being continued from a previous conversation that ran out of context.")).toBe(true);
    });
...
function makeEntry(type, content, timestamp) {
    ...
    return {
        ...
        timestamp: ts,
        sessionId: "test-session",
        message: { role: type === "user" ? "user" : "assistant", content: content },
    };
}
function makeToolEntry(toolName, input, timestamp) {
    return makeEntry("assistant", [
        { type: "tool_use", id: `tool-${Math.random().toString(36).slice(2)}`, name: toolName, input },
    ], timestamp);
}
function makeSession(entries, durationMs = 600_000) {

defmodule HeyiAmAppWeb.UserAuth do
  ...
  @remember_me_cookie "_heyi_am_app_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      HeyiAmAppWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  defp maybe_reissue_user_session_token(conn, user, token_inserted_at) do
    token_age = Date.diff(Date.utc_today(), DateTime.to_date(token_inserted_at))
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      HeyiAmAppWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule HeyiAmAppWeb.PageLive do
        use HeyiAmAppWeb, :live_view

        on_mount {HeyiAmAppWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{HeyiAmAppWeb.UserAuth, :require_authenticated}] do

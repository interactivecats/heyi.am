defmodule HeyiAm.Accounts.DeviceCode do
  ...
  defp generate_user_code do
    ...
    len = length(chars)
    # Use CSPRNG instead of Enum.random (which uses non-cryptographic :rand).
    # len=32 is a power of 2, so rem(byte, 32) has no modulo bias.
    random_char = fn -> Enum.at(chars, rem(:binary.decode_unsigned(:crypto.strong_rand_bytes(1)), len)) end
    part = fn -> Enum.map(1..4, fn _ -> random_char.() end) |> List.to_string() end
    "#{part.()}-#{part.()}"
  end
end
